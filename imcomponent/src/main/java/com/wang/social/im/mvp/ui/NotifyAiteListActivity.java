...
public class NotifyAiteListActivity extends BasicAppNoDiActivity implements IView, BaseAdapter.OnItemClickListener<CommonMsg> {
    ...
    public static void start(Context context) {
        Intent intent = new Intent(context, NotifyAiteListActivity.class);
    ...
    public void initData(@NonNull Bundle savedInstanceState) {
        ...
        titleview.setTitle(getString(R.string.im_notify_aite_title));
    ...
    private void netGetSysMsgList(boolean isFresh) {
        ...
        ApiHelperEx.execute(this, false,
                ApiHelperEx.getService(NotifyService.class).getAiteMsgList(current + 1, size),
                new ErrorHandleSubscriber<BaseJson<BaseListWrap<AiteMsg>>>() {
                    @Override
                    public void onNext(BaseJson<BaseListWrap<AiteMsg>> basejson) {
                        BaseListWrap<AiteMsg> warp = basejson.getData();
                        List<CommonMsg> list = AiteMsg.tans2CommonMsgList(warp.getList());
