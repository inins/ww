...
public class EmoticonsToolBarView extends RelativeLayout {
    ...
    public EmoticonsToolBarView(Context context, AttributeSet attrs) {
        ...
        mInflater.inflate(R.layout.im_view_emotion_toolbar, this);
        this.mContext = context;
        mBtnWidth = (int) context.getResources().getDimension(R.dimen.im_emotion_toolbar_button_width);
        hsv_toolbar = findViewById(R.id.vet_hsv_toolbar);
        ly_tool = findViewById(R.id.vet_ll_toolbar);
    }
    ...
    public void addFixedToolItemView(View view, boolean isRight) {
        ...
        if (view.getId() <= 0) {
            view.setId(isRight ? R.id.im_emotion_toolbar_right : R.id.im_emotion_toolbar_left);
        }
        if (isRight) {
    ...
    protected View getCommonItemToolBtn() {
        return mInflater == null ? null : mInflater.inflate(R.layout.im_emotion_toolbtn_item, null);
    }
    ...
    protected void initItemToolBtn(View toolBtnView, int rec, final PageSetEntity pageSetEntity, OnClickListener onClickListener){
        ImageView iv_icon = toolBtnView.findViewById(R.id.eti_iv_icon);
        if (rec > 0) {
        ...
        if (pageSetEntity != null) {
            iv_icon.setTag(R.id.im_emotion_tag_pageset, pageSetEntity);
            try {
    ...
    protected View getToolBgBtn(View parentView) {
        return  parentView.findViewById(R.id.eti_iv_icon);
    }
    ...
    public void addFixedToolItemView(boolean isRight, int rec, final PageSetEntity pageSetEntity, OnClickListener onClickListener) {
        ...
        if (toolBtnView.getId() <= 0) {
            toolBtnView.setId(isRight ? R.id.im_emotion_toolbar_right : R.id.im_emotion_toolbar_left);
        }
        if (isRight) {
            params.addRule(ALIGN_PARENT_RIGHT);
    ...
    public void setToolBtnSelect(String uuid) {
        ...
        for (int i = 0; i < mToolBtnList.size(); i++) {
            Object object = mToolBtnList.get(i).getTag(R.id.im_emotion_tag_pageset);
            if (object != null && object instanceof PageSetEntity && uuid.equals(((PageSetEntity) object).getUuid())) {
                mToolBtnList.get(i).setBackgroundColor(getResources().getColor(R.color.im_emotion_toolbtn_select));
                select = i;
            } else {
                mToolBtnList.get(i).setBackgroundResource(R.drawable.im_bg_emotion_toolbtn);
            }
        }
