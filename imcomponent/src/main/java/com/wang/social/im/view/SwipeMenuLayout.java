...
public class SwipeMenuLayout extends ViewGroup {
    ...
    private void init(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        //左滑右滑的开关,默认左滑打开菜单
        isLeftSwipe = true;
        TypedArray ta = context.getTheme().obtainStyledAttributes(attrs, R.styleable.im_SwipeMenuLayout, defStyleAttr, 0);
        int count = ta.getIndexCount();
        for (int i = 0; i < count; i++) {
            int attr = ta.getIndex(i);
            if (attr == R.styleable.im_SwipeMenuLayout_im_swipeEnable) {
                isSwipeEnable = ta.getBoolean(attr, true);
            } else if (attr == R.styleable.im_SwipeMenuLayout_im_ios) {
                isIos = ta.getBoolean(attr, true);
            } else if (attr == R.styleable.im_SwipeMenuLayout_im_leftSwipe) {
